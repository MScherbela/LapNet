...
def init_electrons(
    ...
    if (
        sum(atom.charge for atom in molecule) != sum(electrons)
        and given_atomic_spin_configs is None
    ):
        if len(molecule) == 1:
        ...
        atomic_spin_configs = (
            ...
            if given_atomic_spin_configs is None
            else given_atomic_spin_configs
        )
    ...
    return electron_positions + init_width * jax.random.normal(
...
def make_training_step(
    ...
    def step(
        ...
        # Optimization step
        new_params, state, loss, aux_data = optimizer_step(
...
def make_kfac_training_step(
    mcmc_step, damping: float, optimizer: kfac_jax.Optimizer
) -> Step:
...
def train(cfg: ml_collections.ConfigDict, writer_manager=None):
    ...
    if local_batch_size % num_devices != 0:
        raise ValueError(
            ...
            "got batch size {} for {} devices.".format(local_batch_size, num_devices)
        )
    ...
    if cfg.optim.el_partition_num < 0:
        cfg.optim.el_partition_num = find_el_partition_num(
            cfg, start=-cfg.optim.el_partition_num, n=3 * np.sum(cfg.system.electrons)
        )
    ...
    if cfg.pretrain.method == "direct_init" or (
        ...
        hartree_fock.mean_field.mo_coeff = tuple(
            [multi_host.broadcast_to_hosts(x) for x in hartree_fock.mean_field.mo_coeff]
        )

    hf_solution = hartree_fock if cfg.pretrain.method == "direct_init" else None

    (network_init, signed_network, network_options, network_each_det) = (
    ...
    batch_network = jax.vmap(network, in_axes=(None, 0), out_axes=0)  # batched network
    ...
    if ckpt_restore_filename:
        t_init, data, params, opt_state_ckpt, mcmc_width_ckpt, sharded_key_ckpt = (
            checkpoint.restore(ckpt_restore_filename, local_batch_size)
        )
    else:
    ...
    if t_init == 0 and cfg.pretrain.method == "hf" and cfg.pretrain.iterations > 0:
        ...
        sampling_function = (
            pretrain.make_HF_ansatz(hartree_fock, cfg.system.electrons)
            if cfg.pretrain.use_hf_sample
            else batch_network
        )
    ...
    if cfg.system.make_local_energy_fn:
        local_energy_module, local_energy_fn = cfg.system.make_local_energy_fn.rsplit(
            ".", maxsplit=1
        )
    ...
    def learning_rate_schedule(t_: jnp.ndarray) -> jnp.ndarray:
        ...
        orig_lr = cfg.optim.lr.rate * jnp.power(
            (1.0 / (1.0 + fg * (t_ - cfg.optim.lr.warmup) / cfg.optim.lr.delay)),
            cfg.optim.lr.decay,
        )
        linear_lr = (
            cfg.optim.lr.rate
            * t_
            / (cfg.optim.lr.warmup + (cfg.optim.lr.warmup == 0.0))
        )
    ...
    def init_step(cfg, params, data, sharded_key, opt_state_ckpt=None):
        ...
        if not optimizer:
            ...
            step = make_training_step(mcmc_step=mcmc_step, optimizer_step=energy_eval)
            ...
            step = make_kfac_training_step(
                mcmc_step=mcmc_step, damping=cfg.optim.kfac.damping, optimizer=optimizer
            )
        else:
    ...
    step, opt_state, sharded_key = init_step(
        cfg, params, data, sharded_key, opt_state_ckpt=opt_state_ckpt
    )
    ...
    if t_init == 0 or cfg.mcmc.force_burn_in:
        ...
        for t in range(cfg.mcmc.burn_in):
            ...
            data, params, *_ = burn_in_step(
                data=data, params=params, state=None, key=subkeys, mcmc_width=mcmc_width
            )
    ...
    with writer_manager as writer:
        ...
        if cfg.network.det_filter.step > 0 and cfg.optim.optimizer:
            logging.info(
                "number of det will be updated per %d steps",
                cfg.network.det_filter.step,
            )
        else:
        ...
        for t in range(t_init, cfg.optim.iterations):
            ...
            if cfg.network.det_filter.step > 0.0 and cfg.optim.optimizer:
                if t % cfg.network.det_filter.step == 0:
                    params, opt_state, step, num_det, sharded_key = (
                        det_filter.filtering(
                            network_each_det,
                            init_step,
                            params,
                            opt_state,
...
def get_params_initialization_key(deterministic):
    ...
    if deterministic:
        ...
        float_seed = kfac_utils.compute_mean(
            jnp.ones(jax.local_device_count()) * local_seed
        )[0]
...
def make_should_save_ckpt(cfg):
    ...
    def should_save_ckpt(iteration, time_of_last_ckpt):
        return (
            ...
            or iteration >= cfg.optim.iterations - 1
            or (
                cfg.log.save_frequency_in_step > 0
                and iteration % cfg.log.save_frequency_in_step == 0
            )
        )
