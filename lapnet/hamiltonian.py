...
def local_kinetic_energy(
    f: networks.LogWaveFuncLike, use_scan: bool = False, forward_laplacian=True
    ...
    def _forward_lapl_over_f(params, data):
        ...
        return -0.5 * output.get(TupType.LAP) - 0.5 * jnp.sum(
            output.get(TupType.GRAD) ** 2
        )
...
def potential_electron_nuclear(charges: jnp.ndarray, r_ae: jnp.ndarray) -> jnp.ndarray:
...
def potential_nuclear_nuclear(charges: jnp.ndarray, atoms: jnp.ndarray) -> jnp.ndarray:
    """Returns the electron-nuclearpotential.
...
def local_energy(
    ...
    if not with_rng:
        ...
        def _e_l_no_rng(params, data):
            return _e_l(params, None, data)
