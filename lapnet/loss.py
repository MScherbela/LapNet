...
def make_loss(
    ...
    batch_local_energy_with_param = lambda params, data: (
        params,
        batch_local_energy(params, None, data),
    )
    ...
    def pmean_with_mask(value, mask):
        ...
        """
        return kfac_utils.psum_if_pmap(
            jnp.sum(value * mask), axis_name=constants.PMAP_AXIS_NAME
    ...
    def total_energy(
        ...
        if rm_outlier and local_energy_outlier_width > 0.0:
        ...
        return loss, AuxiliaryLossData(
            variance=variance, local_energy=e_l, outlier_mask=mask
        )
    ...
    def total_energy_jvp(primals, tangents):  # pylint: disable=unused-variable
        ...
        if clip_local_energy > 0.0:
            ...
            median = jnp.median(
                jax.lax.all_gather(
                    aux_data.local_energy, axis_name=constants.PMAP_AXIS_NAME
                )
            )
            # median = jnp.median(aux_data.local_energy)
            tv = pmean_with_mask(
                jnp.abs(aux_data.local_energy - median), aux_data.outlier_mask
            )
            diff = jnp.clip(
